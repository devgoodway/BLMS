...
if(empty($_POST['plan_author']) === false){
//시간표 DB에 자동입력
for($i=0;$i<=47;$i++){
...
// 제목
$subject_ovl = '[BLMS]'.$_POST['plan_curriculum']."/".$_POST['plan_subject']." ▶ ".addslashes($_POST['plan_class'])."_".$plan_author[0]."/".$_POST['plan_mentor']."(".$time_str.")"."[".$_POST['plan_classroom']."]"."이 수정되었습니다. 수업을 확인하세요.";
...
<form action="index.php?id=Design_Plan_Update_Admin&season=<?php echo $season_chk;?>" method="post">
...
<br>
  ...
  <tr>
    ...
    <td class="studyplan_table" colspan="3">
      <select class="w3-input w3-white" name="plan_author">
        <?php
          $result_user_mentor_author = mysqli_query($conn, "SELECT * FROM bs_users  WHERE user_season = '".substr($_COOKIE['Season'],0,4)."' ORDER BY user_name");
          echo '<option value="">';
          while( $row_user_author = mysqli_fetch_assoc($result_user_mentor_author)){
              if($row_user_author['user_email']===$data_plan[2]){
              echo '<option value="'.$row_user_author['user_name'].':'.$row_user_author['user_email'].'" selected>'.$row_user_author['user_name'].'['.$row_user_author['user_email'].']</option>';
              }
              else{
                echo '<option value="'.$row_user_author['user_name'].':'.$row_user_author['user_email'].'">'.$row_user_author['user_name'].'['.$row_user_author['user_email'].']</option>';}}
          ?>
      </select></td>
