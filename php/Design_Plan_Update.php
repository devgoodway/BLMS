...
$result = mysqli_query($conn, "SELECT * FROM bs_plan WHERE plan_season = '".$_POST['plan_season']."' AND plan_sub = '".$_POST['plan_curriculum']."/".$_POST['plan_subject']."'");//시즌과 교과목명이 같은 수업을 찾는 쿼리
if($_POST[before][5]!==$data_plan[5] && mysqli_num_rows($result)>1){//교과명이 달라지고 중복된 수업이 있는 경우
/* 다중 수신자 */
$to = $_POST['plan_email'].','.'<YOUR_MASTER2_EMAIL>';
$class = '';
...
<h4><strong>학습계획서 수정 - <?php echo $_COOKIE[Season]; ?></strong></h4>
<h6><strong>시간표 수정</strong></h6>
<form action="index.php?id=Design_Plan_Update&season=<?php echo $season_chk;?>" method="post">
  <input class="w3-input w3-white" type="hidden" name="planId" value="<?php echo $_POST[planId];?>">
<select class="w3-input" name="classroom" onchange="form.submit()">
...
<table class="w3-card-4 w3-bordered w3-centered w3-small">
 <thead>
   <tr>
     <th class=" studyplan_table">시간표</th>
     <th class=" studyplan_table" colspan="2"><?php
     ...
     echo '<input class=" w3-input" style="width:100%;border:0;text-align:center;" type="text" name="plan_classroom" value="'.$data_plan[8].'" readonly>';}
     ?>
     </td>
   </tr>
   <tr>
   ...
   <!-- 시간표 만들기 -->
      ...
      while( $row_plan = mysqli_fetch_assoc($result_plan)){
        if(empty($_POST[classroom])===false){
          //강의실 명을 비교한다.
          if((strstr($row_plan['plan_classroom'],$_POST[classroom])!==false)||(strstr($_POST[classroom],$row_plan['plan_classroom'])!==false)){
            $classroom_time[$i] = $row_plan[plan_time];
            $classroom_mentor[$i] = $row_plan[plan_mentor];
            $classroom_class[$i] = $row_plan[plan_class];
            $i++;
          }}else{
            if((strstr($row_plan['plan_classroom'],$data_plan[8])!==false)||(strstr($data_plan[8],$row_plan['plan_classroom'])!==false)){
              $classroom_time[$i] = $row_plan[plan_time];
              $classroom_mentor[$i] = $row_plan[plan_mentor];
              $classroom_class[$i] = $row_plan[plan_class];
              $i++;
      ...
      for($x = 2; $x <= 12; $x++){
        ...
        $chk = 0;
        for($i=0;$i<count($classroom_time);$i++){
          if(strpos($classroom_time[$i],$day)!==false){
            $chk = 1;
            $mentor_chk = $classroom_mentor[$i];
            $class_chk = $classroom_class[$i];
          }
        }
        //시간표에 체크박스 출력
        if(isset($_POST[classroom])===false){$_POST[classroom]=$data_plan[8];}
        if(($_POST[classroom]=="없음")){
          if(strpos($data_plan[10],$day)!==false){//학습계획서에 저장 되어있는 시간에는 checked
        echo '<input class="w3-input w3-white" style="width:100%;" type="checkbox" name="plan_time'.$chk_cnt.'" value="'.$day.'" checked></td>';}
          elseif(($chk_cnt <> 23)&&($chk_cnt <> 27)&&($chk_cnt <> 31)&&($chk_cnt <> 35)){
        echo '<input class="w3-input w3-white" style="width:100%;" type="checkbox" name="plan_time'.$chk_cnt.'" value="'.$day.'"></td>';}}
        elseif(($chk==1)&&(strpos($class_chk,$data_plan[6])===false)){
        echo $class_chk;
        }
        elseif(strpos($data_plan[10],$day)!==false){//학습계획서에 저장 되어있는 시간에는 checked
        echo '<input class="w3-input w3-white" style="width:100%;" type="checkbox" name="plan_time'.$chk_cnt.'" value="'.$day.'" checked></td>';}
        elseif(($chk_cnt <> 23)&&($chk_cnt <> 27)&&($chk_cnt <> 31)&&($chk_cnt <> 35))
        {
        echo '<input class="w3-input w3-white" style="width:100%;" type="checkbox" name="plan_time'.$chk_cnt.'" value="'.$day.'"></td>';}
...
<h6><strong>학습계획서 수정</strong></h6>

  <table class="w3-card-4 w3-bordered w3-centered w3-small">
    <tbody>
  <tr>
    <th class=" studyplan_table">작성자</th>
    <td class="studyplan_table"><input class="w3-input w3-white" style="width:100%;border:0;text-align:center;" type="text" name="plan_name" value="<?php echo $_COOKIE[Name];?>" readonly></td>
    <td class="studyplan_table" colspan="2"><input class="w3-input w3-white" style="width:100%;border:0;text-align:center;" type="text" name="plan_email" value="<?php echo $_COOKIE[Email];?>" readonly></td>
    <th class=" studyplan_table">멘토</th>
    <td class="studyplan_table">
      <select class="w3-input w3-white" name="plan_mentor" required>
        <?php
          $result_user_mentor_grade = mysqli_query($conn, "SELECT * FROM bs_users  WHERE user_season = '".substr($_COOKIE['Season'],0,4)."' ORDER BY user_name");
          echo '<option value="">';
